...
Revision ID: e9f79802047f
Revises: 
Create Date: 2025-03-22 23:53:47.633935
...
def upgrade() -> None:
    ...
    op.drop_index('ix_tasks_search_task_id', table_name='tasks')
...
def downgrade() -> None:
    ...
    sa.Column('search_task_id', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.Column('question', sa.VARCHAR(length=500), autoincrement=False, nullable=False),
    sa.Column('context', sa.VARCHAR(length=2000), autoincrement=False, nullable=True),
    ...
    op.create_index('ix_tasks_search_task_id', 'tasks', ['search_task_id'], unique=False)
    # ### end Alembic commands ###
