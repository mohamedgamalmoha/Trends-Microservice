...
def test_get_user_token(client):
    ...
    response = client.post("/api/jwt/create/", json={"email": user_data["email"], "password": user_data["password"]})
    data = response.json()
...
def test_verify_user_token(client):
    ...
    response = client.get("/api/jwt/verify/", headers={"Authorization": f"Bearer {access_token}"})
...
def test_get_user_invalid_token(client):
    ...
    response = client.get("/api/users/me/", headers={"Authorization": f"Bearer {access_token}invalid"})
    data = response.json()
...
def test_get_user_not_found(client):
    ...
    response = client.get("/api/users/9999999/", headers={"Authorization": f"Bearer {access_token}"})
    data = response.json()
...
def test_update_user(client):   
    ...
    response = client.put(f"/api/users/{user_id}/", headers={"Authorization": f"Bearer {access_token}"}, json={"first_name": "new_first_name"})
    data = response.json()
...
def test_delete_user(client):
    ...
    response = client.delete(f"/api/users/{user_id}/", headers={"Authorization": f"Bearer {access_token}"})
